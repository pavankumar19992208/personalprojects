...
export const SubjectDetail: React.FC<SubjectDetailProps> = ({
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 animate-in slide-in-from-right duration-300">
      {/* Header */}
      <div className="bg-slate-900/90 border-b border-slate-800 p-6 flex items-center gap-4 sticky top-0 z-20 backdrop-blur-md">
        <button
          onClick={onBack}
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
              ...
              <p className="text-2xl font-bold text-white">{topic.frequency}</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-4">
              {topic.subTopics?.map((sub) => (
                <div
                  key={sub.id}
                  className="bg-slate-900 border border-slate-800 p-5 rounded-xl hover:border-cyan-500/50 transition-all group cursor-pointer"
                >
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="text-lg font-bold text-slate-200 group-hover:text-cyan-400 transition-colors">
                      {sub.title}
                    </h4>
                    <span
